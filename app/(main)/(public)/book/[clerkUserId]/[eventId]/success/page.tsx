...
export default async function SuccessPage({
  ...
  // defininf expected route parameters and search parameters
  params: Promise<{ clerkUserId: string; eventId: string }>;
  searchParams: Promise<{ startTime: string }>;
}) {
  const { clerkUserId, eventId } = await params;
  const { startTime } = await searchParams;
  // quering the database to find the specific active event that matches the user and event ID
  ...
  // fetching the user details from Clerk based on the user ID
  const client = await clerkClient();
  const calendarUser = await client.users.getUser(clerkUserId);

  // converting the received start time string to a JavaScript Date object
  const startTimeDate = new Date(startTime);

  // rendering the success message with event and user details
  return (
