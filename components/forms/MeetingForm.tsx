...
export default function MeetingForm({
  ...
  // initializing form using React Hook Form and Zod schema
  // creating a form using React Hook Form with Zod for validation
  const form = useForm<z.infer<typeof meetingFormSchema>>({
    // using zodResolver to connect Zod schema to React Hook Form
    resolver: zodResolver(meetingFormSchema),

    // setting initial default values for the form fields
    defaultValues: {
      // detect the user's local timezone
      timezone: Intl.DateTimeFormat().resolvedOptions().timeZone,

      // starts with empty fields for guest info
      guestName: "",
      guestEmail: "",
      guestNotes: "",
    },
  ...
  // watching timezone and selected date fields for updates
  const timezone = form.watch("timezone");
  const date = form.watch("date");

  // converting valid times to the selected timezone
  ...
  async function onSubmit(values: z.infer<typeof meetingFormSchema>) {
    try {
      // calling the createMeeting action (assuming it handles success/failure internally)
      ...
      // initializing the path variable to use it later in the finally block
      const path = `/book/${meetingData.clerkUserId}/${
        meetingData.eventId
      }/success?startTime=${meetingData.startTime.toISOString()}`;
      router.push(path);
    } catch (error: any) {
      // handling any error that occurs during the meeting creation
      form.setError("root", {
